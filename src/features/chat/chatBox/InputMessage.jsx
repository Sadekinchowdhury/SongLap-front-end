...
const InputMessage = () => {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex items-center gap-x-5 py-4 sticky top-full p-5 rounded-md bg-[var(--background-color)] shadow-md">
      {/* File Upload */}
      <label className="cursor-pointer">
        <input type="file" className="hidden" onChange={handleFileUpload} />
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 512 512" className="w-6 h-6 fill-[var(--text-color)]">
      ...
      <button type="button" onClick={handleVoiceInput} className="focus:outline-none">
        <svg
          ...
          className="w-6 h-6 fill-[var(--text-color)] hover:fill-red-500"
        >
